# Letsee_mega_fixed_v3.py
# Updated: apply requested changes:
# - Seller role no longer asks payout method immediately; it now asks for deal link and sets step enter_deal_link.
# - Buyer confirmation now sends the exact requested confirmation message.
# - Seller payout (method + address) is requested ONLY after buyer confirms delivery (option 2: method buttons then address).
# - Invite links that are already used return a clear message.
# - A small ephemeral mapping PENDING_PAYOUT is used to track seller payout prompts across chats.
# Preserves the rest of the bot exactly.
...
BOT_TOKEN = "8408620229:AAFA3FprYMcYeT0APAtUexUdl1z38bxcMhc"
BOT_USERNAME = "CastinEscrow_bot"
TERMS_CHANNEL = "@CastinEscrow"

VERIFIED_ESCROWS = {"Tradealer1✅", "rixen10✅", "Nolam11✅"}

PAYMENT_ADDRESSES = {
    "PayPal": "susanzeedy4259@gmail.com",
    "Skrill": "davidchege5942@gmail.com",
    "Crypto": (
        "💳 BNB (BEP20): 0x412930bc47da7a7b5929ae8876ac41e7d39bc9e2\n"
        "💵 USDT (TRC20): TD6TWzH3NW9Phfws6DUDKkpgWLjf9924md"
    ),
    "ACH": (
        "🏦 ACH (USD):\n"
        "Bank: Wells Fargo\n"
        "Account: David Chege\n"
        "Account #: 40630208244298723\n"
        "Routing: 121000248"
    ),
    "Wire": (
        "🌍 Wire (USD):\n"
...
# Ephemeral mapping to track which seller is expected to send a payout address and for which deal.
# Key: seller_user_id -> (deal_id, method)
PENDING_PAYOUT = {}
...
def save_user(username, user_id):
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("INSERT OR REPLACE INTO user_registry (username, user_id, terms_accepted) VALUES (?, ?, COALESCE((SELECT terms_accepted FROM user_registry WHERE username = ?), 0))", (username, user_id, username))
        conn.commit()
        conn.close()
    except Exception as e:
...
def get_user_data(username):
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("SELECT user_id, terms_accepted FROM user_registry WHERE username = ?", (username,))
        row = c.fetchone()
        conn.close()
        if row:
            return {"user_id": row[0], "terms_accepted": bool(row[1])}
        return None
    except Exception as e:
        logger.error(f"❌ Failed to get user  {e}")
        return None

def set_terms_accepted(user_id):
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("UPDATE user_registry SET terms_accepted = 1 WHERE user_id = ?", (user_id,))
        conn.commit()
        conn.close()
    except Exception as e:
        logger.error(f"❌ Failed to set terms accepted: {e}")

def can_create_deal(user_id):
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("SELECT COUNT(*) FROM deals WHERE (buyer_id = ? OR seller_id = ?) AND status IN ('created', 'accepted')", (user_id, user_id))
        active_count = c.fetchone()[0]
        if active_count >= MAX_ACTIVE_DEALS:
            return False, f"❌ You already have {MAX_ACTIVE_DEALS} active deals. Please complete or cancel one before starting a new deal."

        c.execute("SELECT last_deal_creation FROM user_registry WHERE user_id = ?", (user_id,))
        row = c.fetchone()
        if row and row[0]:
            last_creation = row[0]
            if time.time() - last_creation < 3600:  # 1 hour
                c.execute("SELECT COUNT(*) FROM deals WHERE (buyer_id = ? OR seller_id = ?) AND created_at > ?", (user_id, user_id, last_creation))
                recent = c.fetchone()[0]
                if recent >= 10:
                    return False, "❌ Rate limit exceeded: You can create up to 10 deals per hour."

        conn.close()
        return True, ""
    except Exception as e:
        logger.error(f"❌ Failed to check deal limits: {e}")
        return True, ""

def update_last_deal_creation(user_id):
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("UPDATE user_registry SET last_deal_creation = ? WHERE user_id = ?", (time.time(), user_id))
        conn.commit()
        conn.close()
    except Exception as e:
        logger.error(f"❌ Failed to update last deal creation: {e}")
    ...
    try:
        ...
        c.execute("""
            INSERT OR REPLACE INTO deals (
                deal_id, status, buyer_id, seller_id, buyer_username, seller_username,
                escrow_username, description, original_amount, total_with_fee,
                payment_method, seller_payout_method, seller_payout_address, fee_amount,
                created_at, payment_confirmed_at, delivery_confirmed_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            deal["deal_id"],
            deal["status"],
            ...
            deal.get("seller_username"),
            deal["escrow_username"],
            deal["description"],
            deal["original_amount"],
            deal["total_with_fee"],
            deal["payment_method"],
            deal.get("seller_payout_method"),
            deal.get("seller_payout_address"),
            deal["fee_amount"],
            deal.get("created_at", time.time()),
            deal.get("payment_confirmed_at"),
            deal.get("delivery_confirmed_at"),
        ))
        conn.commit()
        conn.close()
    except Exception as e:
...
def log_action(deal_id, action, by, user_id, details=""):
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("INSERT INTO audit_log (deal_id, action, by, user_id, details, timestamp) VALUES (?, ?, ?, ?, ?, ?)",
                  (deal_id, action, by, user_id, details, time.time()))
        conn.commit()
        conn.close()
    except Exception as e:
    ...
    try:
        ...
        if row:
            return {
                ...
                "seller_username": row[5],
                "escrow_username": row[6],
                "description": row[7],
                "original_amount": row[8],
                "total_with_fee": row[9],
                "payment_method": row[10],
                "seller_payout_method": row[11],
                "seller_payout_address": row[12],
                "fee_amount": row[13],
                "created_at": row[14],
                "payment_confirmed_at": row[15],
                "delivery_confirmed_at": row[16],
            }
        return None
    except Exception as e:
...
def get_user_active_deals(user_id):
    try:
        ...
        c.execute("""
            SELECT * FROM deals 
            WHERE (buyer_id = ? OR seller_id = ? OR escrow_username = (
                SELECT username FROM user_registry WHERE user_id = ?
            )) 
            AND status IN ('created', 'accepted', 'payment_confirmed', 'delivery_confirmed')
...
async def check_terms(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    if not user.username:
        if update.effective_message:
            await update.effective_message.reply_text(
                "⚠️ <b>Username Required</b>\n\n"
                "Please set a public Telegram username in Settings > Username, then try again.",
                parse_mode="HTML"
            )
        else:
            try:
                ...
                logger.warning("Unable to send username required warning (no message target).")
        return True
    return False
...
async def show_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await check_terms(update, context):
        return

    is_escrow = user.username in VERIFIED_ESCROWS if user and user.username else False

    if is_escrow:
        deals = get_user_active_deals(user.id)
        status_descriptions = {
            "created": "🟡 Pending Counterparty Acceptance",
            "accepted": "⏳ Awaiting Buyer Payment",
            "payment_confirmed": "📦 Awaiting Seller Delivery",
            "delivery_confirmed": "🔓 Ready for Fund Release"
        }
        deal_list = "\n\n".join([
            f"• <b>{d['deal_id']}</b>\n"
            f"  <b>Status:</b> {status_descriptions.get(d['status'], d['status'])}\n"
            f"  <b>Buyer:</b> @{d['buyer_username']} | <b>Seller:</b> @{d['seller_username']}\n"
            f"  <b>Item:</b> {d['description']} | <b>Amount:</b> {d['original_amount']}"
            for d in deals
        ]) or "You have no active deals at the moment."

        text = (
            f"🛡️ <b>Welcome, Verified Escrow Agent!</b>\n\n"
            f"<b>Your Active Deals ({len(deals)}):</b>\n\n"
            f"{deal_list}\n\n"
            f"Use the buttons below to manage your deals or learn more."
        )
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("📥 View All Active Deals", callback_data="escrow_active")],
            [InlineKeyboardButton("📜 View Deal History", callback_data="escrow_history")],
            [InlineKeyboardButton("ℹ️ How It Works", callback_data="guide:0")],
            [InlineKeyboardButton("🏠 Main Menu", callback_data="main_menu")],
        ])
    else:
        text = (
            "🌟 <b>Welcome to Castin Pro Escrow!</b>\n\n"
            "You’ve entered the most trusted platform for secure Telegram trades.\n\n"
            "<b>How It Works:</b>\n\n"
            "1️⃣ <b>Create a Deal</b> – Choose your role (Buyer or Seller)\n"
            "2️⃣ <b>Agree on Terms</b> – Item, amount, and payment method\n"
            "3️⃣ <b>Assign an Escrow</b> – Only verified professionals\n"
            "4️⃣ <b>Secure Transaction</b> – Buyer pays Escrow, Seller delivers\n"
            "5️⃣ <b>Funds Released</b> – After confirmation or auto-approval\n\n"
            "✅ <b>No scams</b> • 🔒 <b>Private</b> • ⏱️ <b>Time-bound</b>\n\n"
            "Get started below:"
        )
        keyboard = InlineKeyboardMarkup([
